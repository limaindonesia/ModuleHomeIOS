//
//  ConsultationHistoryTests.swift
//  HomeFlow
//
...
final class ConsultationHistoryRemoteTests: XCTestCase {
  
  var sut: ConsultationHistoryRemoteDataSourceLogic!
  ...
  func test_fetchCategories_shouldReturnOneResult() async throws {
    //given
    let service = MockNetworkService()
    service.mockData = jsonResponse
    
    sut = ConsultationHistoryRemoteDataSourceImpl(service: service)
    
    //when
    let consultations = try await sut.requestConsultation(
      headers: [:],
      parameters: [:]
    )
    
    //then
    XCTAssertEqual(consultations.data?.data?.count, 1)
  }
  
  func test_fetchCategories_shouldReturnError() async throws {
    //given
    let service = MockNetworkService()
    var resultError: NetworkErrorMessage = .init(code: 0, description: "")
    
    service.mockError = NetworkErrorMessage(code: -898, description: "gagal lagi")
    sut = ConsultationHistoryRemoteDataSourceImpl(service: service)
    
    //when
    do {
      _ = try await sut.requestConsultation(
        headers: [:],
        parameters: [:]
      )
    }
    catch {
      resultError = error as! NetworkErrorMessage
    }
    
    //then
    XCTAssertEqual(resultError.description, "gagal lagi")
    XCTAssertEqual(resultError.code, -898)
  }
