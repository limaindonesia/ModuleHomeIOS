...
struct LegalFormPaymentView: View {
  
  @ObservedObject var store: LegalFormPaymentStore
  
  init(store: LegalFormPaymentStore) {
    self.store = store
  }
  ...
  var body: some View {
    VStack {
      ...
      StandardHeaderView(title: "Pembayaran") {
        self.store.showWarningPaymentBottomSheet()
      }
      ...
      ZStack {
        ...
        VStack {
          ...
          ScrollView(showsIndicators: false) {
            VStack(spacing: 12) {
              legalFormInfoView(
                orderID: store.entity.orderNumber,
                name: store.entity.title
              )
...
#Preview {
  LegalFormPaymentView(store: .init())
}
